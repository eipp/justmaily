...
class VespaClient {
    constructor(config) {
        ...
        this.client = axios_1.default.create({
            ...
            headers: Object.assign({ 'Content-Type': 'application/json' }, (config.apiKey && { 'X-API-Key': config.apiKey }))
        });
    }
    async query(query) {
        try {
    ...
    async getDocument(type, id) {
        var _a;
        try {
            const response = await this.client.get(`/document/v1/${this.config.tenant}/${type}/docid/${id}`);
    ...
    async updateDocument(type, id, fields) {
        try {
            await this.client.put(`/document/v1/${this.config.tenant}/${type}/docid/${id}`, { fields });
        }
        catch (error) {
    ...
    async deleteDocument(type, id) {
        try {
            await this.client.delete(`/document/v1/${this.config.tenant}/${type}/docid/${id}`);
        }
        catch (error) {
