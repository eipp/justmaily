...
class OptimizationService {
        ...
        try {
            ...
            const variantMetrics = await Promise.all(variants.map(async (variant) => {
                ...
                if (data.length < this.config.modelConfigs.abTest.minSampleSize) {
                    throw new Error(`Insufficient data for variant ${variant}. Minimum sample size: ${this.config.modelConfigs.abTest.minSampleSize}`);
                }
            ...
            const result = {
                ...
                metrics: variantMetrics.map(metric => (Object.assign(Object.assign({}, metric), { improvement: metric.variant === winner.variant
                        ? 0
                        : ((winner.conversionRate - metric.conversionRate) /
                            metric.conversionRate) *
                            100 }))),
                duration: Date.now() - startTime,
                isSignificant: winner.confidence >= this.config.modelConfigs.abTest.confidenceLevel,
                recommendation: this.generateRecommendation(winner, variantMetrics)
            };
            // Cache result
            this.setInCache(cacheKey, result);
            // Record metrics
            await this.metrics.recordLatency('ab_test_analysis', Date.now() - startTime);
    ...
    async predictOptimalDeliveryTime(userId, content, historicalData) {
        ...
        try {
            ...
            // Validate input
            if (historicalData.length < this.config.modelConfigs.deliveryTime.minDataPoints) {
                throw new Error(`Insufficient historical data. Minimum required: ${this.config.modelConfigs.deliveryTime.minDataPoints}`);
            }
            // Extract features
            const features = await this.extractDeliveryTimeFeatures(userId, content, historicalData);
            ...
            this.setInCache(cacheKey, result);
            // Record metrics
            await this.metrics.recordLatency('delivery_time_prediction', Date.now() - startTime);
    ...
    async optimizeContent(content, target, constraints) {
        ...
        try {
            ...
            // Generate variations
            const variations = await this.ai.generateContentVariations(content, target, constraints, this.config.modelConfigs.content.maxVariations);
            // Analyze variations
            const analyzedVariations = await Promise.all(variations.map(async (variation) => {
            ...
            // Find best variation
            const bestVariation = analyzedVariations.reduce((best, current) => current.predictedEngagement > best.predictedEngagement ? current : best);
            const result = {
                variations: analyzedVariations,
                bestVariation: bestVariation.content,
                confidenceScore: await this.calculateConfidenceScore(analyzedVariations),
                insights: await this.generateContentInsights(analyzedVariations)
            };
            // Cache result
            this.setInCache(cacheKey, result);
            // Record metrics
            await this.metrics.recordLatency('content_optimization', Date.now() - startTime);
    ...
    determineWinner(metrics) {
        ...
        const bestMetric = metrics.reduce((best, current) => current.conversionRate > best.conversionRate ? current : best);
    ...
    findCommonImprovements(variations) {
        ...
        return Array.from(improvementTypes).map(type => `Common improvement area: ${type} - ${this.summarizeImprovements(improvements.filter(i => i.type === type))}`);
    }
    analyzeEngagementPatterns(variations) {
        ...
        if (Math.max(...engagementScores) - Math.min(...engagementScores) < 0.1) {
            insights.push('Low variation in engagement scores - consider more diverse content variations');
        }
        if (Math.max(...engagementScores) < 0.5) {
            insights.push('All variations show low predicted engagement - consider major content revisions');
        }
        return insights;
    }
