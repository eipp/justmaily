...
const aiClient = ai_core_1.AIClientFactory.createClient('openai', process.env.OPENAI_API_KEY, process.env.OPENAI_API_ENDPOINT);
class EmailActivities {
    constructor(emailService, analyticsService, aiService, optimizationService, metrics) {
    ...
    async analyzeResults(campaignId) {
        ...
        try {
            ...
            // Calculate rates
            const result = {
                deliveryRate: (metrics.delivery.sent /
                ...
                complaintRate: (metrics.engagement.complaints / metrics.delivery.sent) * 100,
                unsubscribeRate: (metrics.engagement.unsubscribes / metrics.delivery.sent) * 100,
            };
            const duration = Date.now() - startTime;
...
async function analyzeAudience(input) {
    return trackActivity('analyze_audience', async () => {
        ...
        const recipients = await db_1.db.recipients.findMany({
            where: {
                AND: [
                    ((_a = input.audience.demographics) === null || _a === void 0 ? void 0 : _a.age)
                        ? {
                            age: {
                        ...
                        : {},
                    ((_b = input.audience.demographics) === null || _b === void 0 ? void 0 : _b.gender)
                        ? {
                            gender: {
                        ...
                        : {},
                    ((_c = input.audience.demographics) === null || _c === void 0 ? void 0 : _c.location)
                        ? {
                            location: {
                        ...
                        : {},
                    ((_d = input.audience.behavior) === null || _d === void 0 ? void 0 : _d.lastActive)
                        ? {
                            lastActiveAt: {
                        ...
                        : {},
                    ((_e = input.audience.behavior) === null || _e === void 0 ? void 0 : _e.purchaseHistory)
                        ? {
                            purchases: {
                                some: {
                                    type: {
                        ...
                        : {},
                    ((_f = input.audience.behavior) === null || _f === void 0 ? void 0 : _f.interests)
                        ? {
                            interests: {
...
async function personalizeContent(input) {
    return trackActivity('personalize_content', async () => {
        ...
        const variants = await Promise.all(input.audience.segments.map(async (segment) => {
            const result = await aiClient.complete({
                messages: [
                    {
                        role: 'system',
        ...
        const rules = variants.reduce((acc, variant) => {
            acc[variant.segmentId] = {
                conditions: input.audience.segments.find((s) => s.id === variant.segmentId).conditions,
...
async function sendEmail(input) {
    return trackActivity('send_email', async () => {
        const response = await postal_1.postalService.sendEmail({
            ...
            metadata: Object.assign(Object.assign({}, input.metadata), { campaignId: input.campaignId, recipientId: input.recipientId, template: input.template }),
        });
        return {
    ...
    return trackActivity('analyze_results', async () => {
        ...
        const metrics = {
            delivery: {
                ...
                bounces: input.engagementResults.reduce((sum, r) => { var _a; return sum + (((_a = r.data) === null || _a === void 0 ? void 0 : _a.bounces) || 0); }, 0),
            },
            engagement: {
                opens: input.engagementResults.reduce((sum, r) => { var _a; return sum + (((_a = r.data) === null || _a === void 0 ? void 0 : _a.opens) || 0); }, 0),
                clicks: input.engagementResults.reduce((sum, r) => { var _a; return sum + (((_a = r.data) === null || _a === void 0 ? void 0 : _a.clicks) || 0); }, 0),
                complaints: input.engagementResults.reduce((sum, r) => { var _a; return sum + (((_a = r.data) === null || _a === void 0 ? void 0 : _a.complaints) || 0); }, 0),
                unsubscribes: input.engagementResults.reduce((sum, r) => { var _a; return sum + (((_a = r.data) === null || _a === void 0 ? void 0 : _a.unsubscribes) || 0); }, 0),
            },
            rates: {
                deliveryRate: (metrics.delivery.sent /
                    (metrics.delivery.sent + metrics.delivery.failed)) *
        ...
        const recommendations = await aiClient.complete({
            messages: [
                {
                    role: 'system',
                    content: 'You are an expert in providing actionable email marketing recommendations.',
                },
                {
                    role: 'user',
