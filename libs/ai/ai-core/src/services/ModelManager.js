...
class ModelManager {
    ...
    registerModel(config) {
        ...
        this.rateLimiters.set(config.model, new limiter_1.RateLimiter({
            tokensPerInterval: this.rateLimitConfig.requestsPerMinute,
            interval: 60000
        }));
    }
    async selectModel(capabilities, preferredProvider) {
        const availableModels = Array.from(this.modelConfigs.values()).filter(config => capabilities.every(cap => config.capabilities.includes(cap)));
    ...
    async checkRateLimit(model) {
        ...
        if (!limiter)
            throw new Error(`Rate limiter not found for model: ${model}`);
    ...
    async cacheResponse(model, prompt, response) {
        if (!this.cacheConfig.enabled)
            return;
        const cacheKey = `ai:cache:${model}:${Buffer.from(prompt).toString('base64')}`;
        await this.redis.setex(cacheKey, this.cacheConfig.ttlSeconds, JSON.stringify(response));
    }
