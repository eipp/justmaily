...
class AIMetrics {
    ...
    getContentMetrics() {
        const totalRequests = this.requestCounter.get().values.reduce((sum, value) => sum + value.value, 0);
        const totalErrors = this.errorCounter.get().values.reduce((sum, value) => sum + value.value, 0);
        const latencySum = this.latencyHistogram.get().sum;
        const latencyCount = this.latencyHistogram.get().count;
        const cacheHits = this.cacheHitCounter.get().values.reduce((sum, value) => sum + value.value, 0);
        const cacheMisses = this.cacheMissCounter.get().values.reduce((sum, value) => sum + value.value, 0);
        return {
