...
export class APIKeyManager {
  ...
  async createAPIKey(params: {
    ...
    await this.metrics.incrementCounter('api_key_created')
  ...
  async validateAPIKey(apiKey: string): Promise<{
    ...
    try {
      ...
      if (!apiKey?.startsWith(this.config.prefix)) {
        await this.metrics.recordAuthFailure('api_key_validation_prefix')
        return { valid: false }
      }
      ...
      if (key.length < this.config.minLength || !/^[a-f0-9]+$/i.test(key)) {
        await this.metrics.recordAuthFailure('api_key_validation_format')
        return { valid: false }
      }
      ...
      if (!keyId) {
        await this.metrics.recordAuthFailure('api_key_validation_not_found')
        await this.security.logAuditEvent({
      ...
      if (!apiKeyData) {
        await this.metrics.recordAuthFailure('api_key_validation_data_not_found')
        await this.security.logAuditEvent({
          action: 'api_key_validation_failed',
          userId: 'system',
          resource: 'api_key',
      ...
      if (apiKeyData.expiresAt && new Date(apiKeyData.expiresAt) < new Date()) {
        await this.metrics.recordAuthFailure('api_key_expired')
        await this.security.logAuditEvent({
          action: 'api_key_validation_failed',
      ...
      if (this.config.rateLimit) {
        ...
        if (isRateLimited) {
          await this.metrics.recordAuthFailure('api_key_rate_limited')
          await this.security.logAuditEvent({
            action: 'api_key_validation_failed',
            userId: apiKeyData.ownerId,
            resource: 'api_key',
      ...
      await this.metrics.recordAuthSuccess('api_key_validation')
      ...
      await this.metrics.recordAuthFailure('api_key_validation_error')
      await this.security.logAuditEvent({
  ...
  async revokeAPIKey(id: string, ownerId: string): Promise<boolean> {
    try {
      ...
      await this.metrics.incrementCounter('api_key_revoked')

      return true
    } catch (error) {
      await this.metrics.recordError('api_key_revocation', error.message)
      return false
    }
  }
  ...
  private async checkRateLimit(keyId: string): Promise<boolean> {
    ...
    if (err) {
      await this.metrics.recordError('rate_limit_check', err.message)
  ...
  async listAPIKeys(
    ...
    try {
      ...
      const sanitizedKeys = paginatedKeys.map(({ hashedKey, ...rest }) => rest)
      ...
      await this.metrics.recordError('api_key_list', error.message)
      throw error
    }
  }
} 