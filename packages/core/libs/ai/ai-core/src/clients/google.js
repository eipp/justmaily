...
class GoogleAIClient extends base_1.BaseAIClient {
  ...
  async complete(request) {
    ...
    const completion = await (0, p_retry_1.default)(
      async () => {
        const model = this.client.getGenerativeModel({
          model: request.modelConfig.model,
        });
        const response = await (0, p_timeout_1.default)(
      ...
      },
      {
        ...
        onFailedAttempt: (error) => {
          console.error(
            `Google AI API request failed (attempt ${error.attemptNumber}): ${error.message}`,
          );
        },
      },
    );
  ...
  streamComplete(request) {
    return __asyncGenerator(this, arguments, function* streamComplete_1() {
      ...
      try {
        for (
          var _f = true, _g = __asyncValues(response.stream), _h;
          (_h = yield __await(_g.next())), (_a = _h.done), !_a;
          _f = true
        ) {
          _c = _h.value;
          _f = false;
          ...
          yield yield __await({
            id:
              ((_d = chunk.promptFeedback) === null || _d === void 0
                ? void 0
                : _d.blockReason) || 'chunk',
            model: request.modelConfig.model,
            choices: [
              {
                index: 0,
                ...
                finishReason:
                  ((_e = chunk.promptFeedback) === null || _e === void 0
                    ? void 0
                    : _e.blockReason) || null,
              },
            ],
