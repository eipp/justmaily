...
(0, vitest_1.describe)('BaseAIClient', () => {
  ...
  (0, vitest_1.describe)('complete', () => {
    ...
    (0, vitest_1.it)('should handle errors and record metrics', async () => {
      ...
      vitest_1.vi
        .spyOn(client, 'generateCompletion')
        .mockRejectedValueOnce(error);
      await (0, vitest_1.expect)(
        client.complete('test prompt'),
      ).rejects.toThrow('API error');
      (0, vitest_1.expect)(client.testMetrics).toHaveBeenCalledWith(
  ...
  (0, vitest_1.describe)('embed', () => {
    ...
    (0, vitest_1.it)(
      'should handle errors in embedding generation',
      async () => {
        const error = new Error('Embedding error');
        vitest_1.vi
          .spyOn(client, 'generateEmbedding')
          .mockRejectedValueOnce(error);
        await (0, vitest_1.expect)(client.embed('test text')).rejects.toThrow(
          'Embedding error',
        );
      },
    );
